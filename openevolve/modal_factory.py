...
def register_modal_components(config: "Config") -> None:
    ...
    logger.debug("Registering Modal components with configuration")
    ...
    hub_max_containers = get_config_value(
        "OE_HUB_MAX_CONTAINERS", config.modal.hub_max_containers, int
    )
    hub_max_concurrent = get_config_value(
        "OE_HUB_MAX_CONCURRENT", config.modal.hub_max_concurrent_requests, int
    )
    ...
    # ----- Evolution Worker -----
    worker_min_containers = get_config_value(
        "OE_WORKER_MIN_CONTAINERS", config.modal.worker_min_containers, int
    )
    worker_max_containers = get_config_value(
        "OE_WORKER_MAX_CONTAINERS", config.modal.worker_max_containers, int
    )
    worker_timeout = get_config_value(
        "OE_WORKER_TIMEOUT", config.modal.worker_timeout, int
    )
    ...
    # ----- LLM Generation -----
    llm_min_containers = get_config_value(
        "OE_LLM_MIN_CONTAINERS", config.modal.llm_min_containers, int
    )
    llm_max_containers = get_config_value(
        "OE_LLM_MAX_CONTAINERS", config.modal.llm_max_containers, int
    )
    ...
    # ----- Evaluation -----
    eval_min_containers = get_config_value(
        "OE_EVAL_MIN_CONTAINERS", config.modal.eval_min_containers, int
    )
    eval_max_containers = get_config_value(
        "OE_EVAL_MAX_CONTAINERS", config.modal.eval_max_containers, int
    )
    ...
    eval_max_concurrent = get_config_value(
        "OE_EVAL_MAX_CONCURRENT", config.modal.eval_max_concurrent_inputs, int
    )
    eval_target_concurrent = get_config_value(
        "OE_EVAL_TARGET_CONCURRENT", config.modal.eval_target_concurrent_inputs, int
    )
    ...
    logger.debug(f"Successfully registered Modal components:")
    logger.debug(
        f"  ControllerHub: timeout={hub_timeout}, max_containers={hub_max_containers}, max_concurrent={hub_max_concurrent}"
    )
    logger.debug(
        f"  evolve_worker: min={worker_min_containers}, max={worker_max_containers}, timeout={worker_timeout}"
    )
    logger.debug(
        f"  llm_generate: min={llm_min_containers}, max={llm_max_containers}, timeout={llm_timeout}"
    )
    logger.debug(
        f"  modal_evaluate: min={eval_min_containers}, max={eval_max_containers}, timeout={eval_timeout}, retries={eval_retries}"
    )
    logger.debug(
        f"  modal_evaluate concurrency: max={eval_max_concurrent}, target={eval_target_concurrent}"
    )
