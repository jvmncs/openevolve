...
async def _direct_evaluate(
    ...
    if sandbox_executor:
        ...
        return await sandbox_executor.evaluate_program(program_code, program_id)
...
async def _cascade_evaluate(
    ...
    try:
        ...
        if spec is None or spec.loader is None:
            return await _direct_evaluate(program_path, config, evaluation_file, sandbox_executor)
        ...
        # Check threshold
        if not _passes_threshold(stage1_eval_result.metrics, config.cascade_thresholds[0]):
